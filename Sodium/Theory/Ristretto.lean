...
def spec : PrimeOrderGroup where
  ...
  uniformBytes := 64
  nonReducedBytes := 64
  mulBase n := if mask n % order = 0 then none else some (enc 32 (mask n % order))
  mul n p :=
    if dec p < order then
  ...
  fromUniform u := enc 32 (dec u % order)
  validPoint p := decide (dec p < order)
  scalarReduced s := decide (dec s < order)
  scalarReduce s := enc 32 (dec s % order)
  scalarAdd x y := enc 32 ((dec x + dec y) % 256 ^ 32 % order)
  scalarMul x y := enc 32 (dec x * dec y % order)
  scalarNeg x := enc 32 ((order - dec x % order) % order)

...
def testKdfDeriveFromKey : IO Unit := do
  try
    let masterKey ← kdfKeygen
    let context := "testctx\x00".toUTF8
    let subkey1 ← kdfDeriveFromKey 32 1 context masterKey
    let subkey2 ← kdfDeriveFromKey 32 2 context masterKey
...
def testHkdfSha256 : IO Unit := do
  try
    let prk ← hkdfSha256Keygen
    let salt := "salt".toUTF8
    let ikm := "input key material".toUTF8
    let context := "test context".toUTF8
    
    let extractedPrk ← hkdfSha256Extract salt ikm
    let expandedKey ← hkdfSha256Expand 32 context extractedPrk
...
def testHkdfSha512 : IO Unit := do
  try
    let prk ← hkdfSha512Keygen
    let salt := "salt512".toUTF8
    let ikm := "input key material 512".toUTF8
    let context := "test context 512".toUTF8
    
    let extractedPrk ← hkdfSha512Extract salt ikm
    let expandedKey ← hkdfSha512Expand 64 context extractedPrk
...
def testInvalidInputs : IO Unit := do
  ...
  try
    let _ ← kdfDeriveFromKey 32 1 validContext invalidKey
  ...
  try
    let _ ← kdfDeriveFromKey 32 1 invalidContext validKey
  ...
  try
    let _ ← kdfDeriveFromKey 100 1 validContext validKey
